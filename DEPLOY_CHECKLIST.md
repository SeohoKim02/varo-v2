...
- [ ] 루트 `requirements.txt`의 `torch>=2.2,<3`이 설치되어 DQN CPU 학습이 가능한지 확인한다.
- [ ] Streamlit Cloud 현재 기본 Python 3.12 대신 고급 설정에서 로컬 검증 환경과 같은 3.11을 선택한다.
- [ ] torchvision·torchaudio·CUDA 전용 패키지를 배포 의존성에 추가하지 않는다.
...
- [ ] DQN 탭 상단에 원본/균형형 선택, 에피소드 입력, `DQN 학습 실행` primary 버튼이 보인다.
- [ ] PyTorch 버전과 CPU/GPU 실행 장치가 표시되고, 미설치 시 이유와 비활성 버튼이 보인다.
- [ ] DQN 샘플 10개 일괄 검증과 원본 vs 균형형 비교 버튼이 단건 학습 영역과 구분된다.
...
- [ ] Cloud 파일 저장 실패 시에도 현재 세션의 학습 결과 요약이 표시되고 앱이 중단되지 않는다.
