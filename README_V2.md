...
- `DQN 학습`: 현재 적용 데이터 단건 학습
- `DQN 샘플 10개 학습`: 샘플 01~10 순차 학습
- `원본 vs 균형형 비교`: 같은 데이터의 두 label 분포 비교
...
- 모델·결과: `outputs/dqn/`
- 균형형 파생 엑셀: `outputs/dqn/balanced_samples/`
...
PyTorch는 선택 의존성이다. 설치되지 않아도 VHS·Greedy·Pareto·업로드·시뮬레이션은 실행되며 DQN 학습 버튼만 제한된다.
...
- 가벼운 데모 배포: `requirements.txt` 사용. 업로드, VHS, Greedy, Pareto, 시뮬레이션과 경로 상세를 실행한다.
- DQN 학습 포함 배포: `requirements-dqn.txt` 사용. 기본 패키지에 PyTorch를 추가해 단건 학습과 원본 vs 균형형 비교를 실행한다.

PyTorch는 용량과 시작 시간이 크므로 기본 requirements에서 제외했다. PyTorch가 없는 환경에서도 앱은 실행되고 DQN 학습 버튼만 제한된다.

앱 진입점은 `app_v2.py`이며 `.streamlit/config.toml`에 밝은 테마가 고정돼 있다.

Streamlit Community Cloud 등에서는 `app_v2.py`를 진입점으로 지정한다. 가벼운 데모는 `requirements.txt`, DQN 학습 포함 환경은 `requirements-dqn.txt`를 사용한다.
