...
class PipelineResult:
    ...
    weight_sensitivity_analysis: Dict[str, Any] = field(default_factory=dict)
...
def run_analysis_pipeline(
    ...
    if auto_vhs.analysis:
        ...
        result.pareto_analysis = dict(auto_vhs.analysis.get("pareto") or {})
        result.weight_sensitivity_analysis = dict(auto_vhs.analysis.get("weight_sensitivity") or {})
    ...
    # Plain-status summaries for the UI (no weights/formulas exposed on screen).
    result.stability_analysis_status = recommendation_stability(result.weight_sensitivity_analysis)
    result.confidence_status = recommendation_confidence(
        standard_recommendations, result.to_dict(), result.stability_analysis_status
    )

    # Unified candidate judgment log: one record per candidate (kept AND excluded)
    ...
    result.candidate_ledger = build_candidate_ledger(
        ...
        reasons_by_route=result.reason_analysis.get("reasons"),
        stability_status=result.stability_analysis_status.get("status"),
    )
...
def build_v2_state(
    ...
    result = run_analysis_pipeline(
        data, raw_data=raw_data, source_metadata=source_metadata, data_signature=data_signature,
    )
    return {
        "validation": validation,
