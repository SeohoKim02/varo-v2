...
* Uploads are a two-phase intake: ``data_application.prepare_pending_data`` only
  *inspects* a file into ``pending_*`` (any status), and ``commit_pending_data``
  applies only the validated usable data on an explicit button click. So a ``pending_*`` set
...
def _build(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    pending_unusable = bool(pending_error) or (pending_present and not bool(state.get("pending_apply_allowed")))
    ...
    if state.get("analysis_run_required"):
        return {
            **base,
