...
class SimulationSampleWorkbookTests(unittest.TestCase):
    ...
    def test_layout_is_radial_and_keeps_nodes_inside_the_canvas(self):
        nodes = [
        ...
        self.assertEqual(layout.canvas["layout_mode"], "radial")
        width, height = layout.canvas["width"], layout.canvas["height"]
        for node in layout.dcs + layout.stores:
            self.assertGreaterEqual(node["x"], 0)
            self.assertLessEqual(node["x"], width)
            self.assertGreaterEqual(node["y"], 0)
            self.assertLessEqual(node["y"], height)
...
class SimulationSamplePageTests(unittest.TestCase):
    ...
    def test_data_management_sample_selector_loads_selected_sample(self):
        app_path = str(Path(__file__).resolve().parents[1] / "app_v2.py")
        app = AppTest.from_file(app_path, default_timeout=120)
        app.run()
        app.session_state["current_menu"] = "데이터 관리"
        app.run()
        selector = next(item for item in app.selectbox if item.key == "simulation_sample_select")
        selector.select("10점포 2DC 샘플").run()
        button = next(item for item in app.button if item.key == "load_simulation_sample")
        button.click().run()
        self.assertFalse(app.exception)
